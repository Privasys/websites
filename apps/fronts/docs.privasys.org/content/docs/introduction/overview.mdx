...
A mobile FIDO2 authenticator for iOS and Android with built-in RA-TLS verification. The wallet verifies that a server is running inside a genuine hardware enclave before your private key is used. Authentication keys are stored in the device's secure hardware (Secure Enclave on iOS, StrongBox on Android) and never leave the device. Push notifications from enclaves are end-to-end encrypted.

[Wallet documentation](/solutions/wallet/overview)
...
<Cards>
  ...
  <Card title="Privasys Wallet" href="/solutions/wallet/overview">
    Mobile FIDO2 authenticator with RA-TLS verification for hardware-attested enclaves.
  </Card>
</Cards>
