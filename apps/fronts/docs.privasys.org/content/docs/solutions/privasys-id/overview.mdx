---
title: Overview
description: The identity and authentication layer for Privasys — wallet, identity provider, browser SDK, and sealed-session transport for non-RA-TLS-capable clients.
---
...
### [Sealed Sessions](/technology/sealed-session)

The three components above combine into the platform's sealed-session mechanism: attestation-verified, end-to-end-confidential transport for clients that cannot speak RA-TLS, primarily browsers and WebViews. The user's wallet does the verification once over RA-TLS, the IdP signs a JWT bound to that verification, and the browser inherits the result through a session key derived in the iframe. The concept, the wire protocol, the trust argument, and the session lifecycle are presented in detail under [Technology > Sealed Session](/technology/sealed-session).
...
- **You're shipping a browser-based UI on top of a confidential workload.** Read [Technology > Sealed Session](/technology/sealed-session).
