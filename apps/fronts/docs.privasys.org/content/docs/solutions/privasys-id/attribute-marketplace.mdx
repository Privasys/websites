...
stateDiagram-v2
    direction LR
    [*] --> Reserved: sign-in with attribute request<br/>credits reserved, voucher minted
    Reserved --> Settled: enclave produces the disclosure<br/>relying party is charged
    Reserved --> Released: user declines, abandons,<br/>or the voucher expires
    Settled --> [*]: provider earns 85%
    Released --> [*]: nothing charged, no trace
```
