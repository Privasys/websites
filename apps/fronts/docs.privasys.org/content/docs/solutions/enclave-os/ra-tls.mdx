...
| `1.3.6.1.4.1.65230.2.4` | Runtime Version Hash | SHA-256 of the runtime version (reserved for Wasmtime version). |
| `1.3.6.1.4.1.65230.2.5` | Combined Workloads Hash | SHA-256 of all loaded WASM bytecode (registered by the WASM module). |
| `1.3.6.1.4.1.65230.2.7` | Attestation Servers Hash | SHA-256 of the sorted attestation server URL list. |
...
│   ├── 2.4                          Runtime version hash (reserved)
...
| `1.3.6.1.4.1.65230.2.5` | WASM | SHA-256 of combined WASM bytecode (32 bytes) |
| `1.3.6.1.4.1.65230.2.7` | Core | SHA-256 of attestation server URLs (32 bytes) |
...
| **Per-app** | App leaf cert (via SNI) | App Code Hash (`3.2`), App Config Merkle Root (`3.1`), App Key Source (`3.4`), app-specific (`3.*`) |

The enclave-wide certificate still contains the **combined workloads hash** (OID `1.3.6.1.4.1.65230.2.5`) for clients that want a single check covering all loaded apps without SNI routing.
...
6. **For per-app**: Check OID `3.2` (app code hash), OID `3.1` (app config Merkle root), and OID `3.4` (app key source) on the app leaf cert.
