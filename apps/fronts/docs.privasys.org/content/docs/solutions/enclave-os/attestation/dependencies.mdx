...
graph LR
  subgraph A["Document service (app A)"]
    A61["cert OID 6.1: pinned set<br/>stamped by the measured runtime"]
  end
  subgraph B["Confidential inference (app B)"]
    B61["its own 6.1 set,<br/>folded into identity(B)"]
  end
  C["Confidential vector store (app C)"]
  X["Any undeclared enclave,<br/>even a genuine one"]

  A -->|"mutual RA-TLS: peer must match<br/>measurement + code hash + app id"| B
