...
The **Config Merkle Tree** closes this gap. Every configuration input is hashed into a leaf of a binary Merkle tree. The root hash is embedded in the [RA-TLS certificate](/solutions/enclave-os/attestation/ra-tls) as [OID `1.3.6.1.4.1.65230.2.1`](/solutions/enclave-os/attestation/oids) (`5.1` for a per-workload tree).
...
- **Full audit**: Request the configuration manifest from the [attestation server](/technology/attestation/attestation-server) and recompute the Merkle root to verify all inputs.
...
|-------|-------------|-----------------|----------------|
| **Platform-wide** | Platform leaf (Mini) or management cert (Virtual) | `2.1` | CA material, runtime config, combined workloads hash |
| **Per-workload** | App leaf cert (Mini) or container cert (Virtual) | `5.1` | Workload-specific code hash, config, key source |

A client connecting via [SNI routing](/solutions/enclave-os/attestation/ra-tls#per-workload-certificates-and-sni-routing) receives the per-workload certificate and can verify both the workload's own Merkle root (`5.1`) and the platform-level root (`2.1`) from the platform certificate.
