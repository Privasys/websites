...
Pinning names machines; it does not prove they are physically protected. The owner decides which machines belong on the list. Intel's Platform Ownership Endorsements will let a platform prove its owner directly; the certificate scheme reserves slot 1.4 for the owner they attest and evidence type 8.5 for the endorsement itself, and the allow-list is the interim mechanism until a distribution channel exists.
