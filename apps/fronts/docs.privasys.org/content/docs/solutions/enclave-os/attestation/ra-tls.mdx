...
5. **Verify the quote** against the hardware vendor's attestation infrastructure through the [attestation server](/technology/attestation/attestation-server): signature, certificate revocation and TCB. Check the measurements (MRENCLAVE or MRTD + RTMRs) against known-good values, and the runtime-measured extensions against the quote's measurements where your policy pins them. When your policy names the machines the enclave may run on, check the reported platform identity against it (see [platform pinning](/solutions/enclave-os/attestation/platform-pinning)).
...
| **Trusted time** | The runtime stamps `quote_time` and leaf validity, and judges the peers it verifies, on its [trusted time](/solutions/enclave-os/trusted-time), never on the host clock alone | Both |
