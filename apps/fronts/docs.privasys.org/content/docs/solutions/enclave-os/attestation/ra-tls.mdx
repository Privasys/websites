...
The trade-off is that the quote could theoretically be replayed onto a different session if the enclave's TLS key were extracted, since the binding above commits to the public key but not to the negotiated session secrets. The key never leaves the TEE, so this requires breaking the hardware's memory isolation, but for defence in depth we offer **channel binding** below.
...
Challenge mode always binds the quote to the specific TLS session. The enclave folds a 32-byte **channel binder** (derived from the shared TLS handshake key schedule with `HKDF-Expand-Label(client_handshake_traffic_secret, "privasys-ratls-binder-v1", ...)`) into `ReportData`:

```
ReportData = SHA-512( SHA-256(SPKI_DER) || nonce || binder )
```

Because the binder is derived from this session's Diffie-Hellman secret and the quote is signed by the hardware attestation key (which never leaves the CPU), a quote cannot be relayed onto a *different* session — even if the leaf TLS key were extracted. The client recomputes the binder from its own copy of the key schedule and checks it against the quote before sending any data, failing closed on a mismatch. There is no negotiation and no un-bound challenge variant: a challenge quote that does not commit to the live session is rejected.
...
| **TLS stack** | rustls (pure Rust, inside SGX) | Caddy RA-TLS module (Go, Caddy `tls.issuance`) |
...
TLS is handled by Caddy's RA-TLS module (a Caddy reverse proxy module). Caddy terminates TLS and forwards plaintext to containers on localhost. Certificate selection uses SNI, with each container hostname mapping to its own RA-TLS certificate. Certificates are cached by certmagic and auto-renewed when they expire.
...
| **Per-workload OIDs** | App code hash (`3.2`), config root (`3.1`), key source (`3.4`), app id (`3.6`) | Image digest (`3.2`), image ref (`3.3`), config root (`3.1`), volume encryption (`3.4`), app id (`3.6`) |
...
graph LR
  ...
  subgraph TEE["One enclave, one TLS endpoint"]
    ...
    T --> LA["Leaf for app A<br/>3.1, 3.2, 3.4, 3.6 of A only"]
    T --> LB["Leaf for app B<br/>3.1, 3.2, 3.4, 3.6 of B only"]
    T --> LP["Platform cert<br/>combined workloads hash (2.5)"]
  end
...
Root CA (operator-provisioned)
 +-- Intermediary CA (sealed inside enclave)
      +-- Enclave CA Cert (attested, SGX quote lives here)
            ...
            |     |-- OID 1.3.6.1.4.1.65230.3.6  App Id (MR_APP sealing)
...
6. **Check the image profile** (Virtual): the Image Profile extension (OID `2.8`) must be `"production"`. Dev-profile images (built with SSH and debug tools) are rejected unless the verifier explicitly opts in.
7. **Check configuration OIDs**: Verify the Merkle root, workload hashes, or individual OID values against expected values.
