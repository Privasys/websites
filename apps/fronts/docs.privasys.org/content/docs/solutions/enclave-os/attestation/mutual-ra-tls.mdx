...
sequenceDiagram
  ...
  A->>B: ClientHello [+ nonce, extension 0xFFBB]
  B-->>A: ServerHello + Server Certificate<br/>(server RA-TLS cert with TEE quote,<br/>client nonce bound into ReportData)
  B-->>A: CertificateRequest [+ nonce, extension 0xFFBB]<br/>(server challenge for the client)
  A->>B: Client Certificate<br/>(client RA-TLS cert with TEE quote,<br/>server nonce bound into ReportData)
  A->>B: Finished
  B-->>A: Finished
  Note over A,B: both sides verified, encrypted channel
```
...
- **Go**: [Privasys/go](https://github.com/Privasys/go/tree/release-branch.go1.26) adds an `RATLSChallenge` field to `tls.CertificateRequestInfo`, carrying the server's nonce. Requires the `ratls` build tag.
