...
| Aspect | Detail |
|--------|--------|
| **Transport** | RA-TLS (TLS 1.3 with an enclave-issued certificate and an SGX quote served on the connection) |
...
This page describes the **enclave-side** MCP tool generation built into Enclave OS Mini. For the broader concept of wrapping MCP in RA-TLS for mutual attestation between agents and tool servers, see [Attested MCP](/technology/confidential-ai/attested-mcp).
...
> **Container applications** also support MCP tool discovery via a `privasys.json` manifest. See [Deploy Containers — MCP tool discovery](/solutions/platform/developer-platform/deploy-container#mcp-tool-discovery) for details.
