...
| **Host replays old ciphertext** | Not detected by the KV store itself. For state that must be fresh, use the [Merkle store](/docs/solutions/enclave-os/enclave-os-mini/merkle-store): reads verify against an in-memory root the host cannot roll back. |
| **Host deletes keys** | Not detected by the KV store itself (an absent key looks absent). The [Merkle store](/docs/solutions/enclave-os/enclave-os-mini/merkle-store) proves both inclusion and absence against the root. |
