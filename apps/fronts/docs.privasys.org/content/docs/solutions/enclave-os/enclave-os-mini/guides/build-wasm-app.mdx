...
See [WASM Runtime](/solutions/enclave-os/enclave-os-mini/wasm-runtime) for details on capabilities and isolation.
...
Using any [RA-TLS client](/solutions/platform/verification-libraries):
...
4. The code hash is added as a leaf in the [Config Merkle tree](/solutions/enclave-os/attestation/merkle-tree).
...
Once loaded, the WASM app's code hash is embedded in its RA-TLS leaf certificate (served via SNI):

| OID | Content |
|-----|---------|
| `1.3.6.1.4.1.65230.4.2` | SHA-256 code digest of the WASM binary |
| `1.3.6.1.4.1.65230.5.1` | Per-app config Merkle root |
| `1.3.6.1.4.1.65230.2.4` | Combined workloads hash (platform certificate, covers all apps) |
...
To build the enclave with WASM runtime enabled, see [Deploy Enclave OS](/solutions/enclave-os/enclave-os-mini/guides/deploy#build-with-wasm-runtime).

## Deploy to the Privasys Platform

The fastest way to deploy your WASM app is through the [Developer Platform](https://developer.privasys.org). Instead of building Enclave OS from source and managing SGX hardware yourself, the platform handles everything:

1. **Sign in** at [developer.privasys.org](https://developer.privasys.org) with your GitHub account
...
See the [Developer Platform guide](/solutions/platform/developer-platform/deploy-wasm) for the full deployment workflow.
