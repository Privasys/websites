...
> If you want a managed deployment experience instead, the [Developer Platform](https://developer.privasys.org) handles hardware allocation, builds, and attestation for you. See [Getting Started](/solutions/platform/developer-platform/getting-started).
...
See the [Layer 4 Proxy Guide](/solutions/enclave-os/enclave-os-mini/guides/layer4-proxy) for Caddy (caddy-l4) and HAProxy configurations.

## Loading WASM Apps

The enclave starts **empty**; no WASM apps are compiled in. Apps are loaded at runtime over the RA-TLS connection. See [Build a WASM App](/solutions/enclave-os/enclave-os-mini/guides/build-wasm-app) for the full workflow.
...
Use [RA-TLS clients](/solutions/platform/verification-libraries) to connect to the enclave with verification:
...
- `1.3.6.1.4.1.65230.2.4`: Combined Workloads Hash (WASM apps)
