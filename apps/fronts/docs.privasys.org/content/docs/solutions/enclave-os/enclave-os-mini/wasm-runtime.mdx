...
- **Memory isolation**: Each component has its own linear memory, with no shared memory between applications.
...
2. Stored as a leaf in the [configuration Merkle tree](/solutions/enclave-os/attestation/merkle-tree).
3. Embedded in the app's RA-TLS leaf certificate as a custom X.509 extension (OID `1.3.6.1.4.1.65230.4.2`); the platform certificate additionally carries the combined workloads hash (OID `1.3.6.1.4.1.65230.2.4`).
...
Every WASM application deployed in the enclave is automatically exposed as an **MCP (Model Context Protocol) tool server**. The enclave introspects each module's WIT types and `///` doc comments to generate a standard MCP tool manifest — one tool per exported function, with JSON Schema inputs derived from the WIT parameter types.

This means AI agents can discover and call your enclave functions through the standard MCP protocol, with full hardware attestation on every connection. No separate MCP server to build, no manifest to maintain.

See [MCP Tools](/solutions/enclave-os/enclave-os-mini/mcp-tools) for the full protocol reference, manifest structure, and WIT-to-JSON-Schema mapping.

## Waiting Without Blocking

A guest's outbound HTTPS call looks blocking to the guest, but the enclave does not block: every guest call runs on a wasmtime fiber, so the runtime suspends the guest while its request waits on the network and serves other applications meanwhile. Long computations yield in the same way every million units of fuel. See [Coroutines](/solutions/enclave-os/enclave-os-mini/coroutines) for how this works and what it measures.
...
| **Limited stack size** | Enclave stack is configured in `enclave.config.xml`. wasmtime bounds the guest's own stack depth, and fuel metering ends runaway WASM code. |
| **No threads to spare** | The enclave serves every app from one thread. Guest calls run on fibers with heap-allocated stacks, registered with the SGX runtime so that its exception handling accepts them, which lets a waiting call step aside (see [Coroutines](/solutions/enclave-os/enclave-os-mini/coroutines)). |
