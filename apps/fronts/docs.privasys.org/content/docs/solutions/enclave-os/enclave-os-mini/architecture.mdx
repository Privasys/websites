...
1. **RA-TLS Ingress.** Accept incoming TCP connections authenticated via RA-TLS (TLS 1.3 with enclave-issued X.509 certificates; the SGX quote is served on the connection by the evidence exchange).
2. **HTTPS Egress.** Make outbound HTTPS requests from inside the enclave (TLS termination inside the enclave, sockets owned by the host). A request waiting on the network is suspended and the enclave keeps serving the others (see [Coroutines](/solutions/enclave-os/enclave-os-mini/coroutines)).
...
4. **Authenticated Merkle Store.** A versioned sparse Merkle tree over the same host storage: reads verified against an in-enclave root, inclusion/absence proofs, pruning, and the state root embedded in every RA-TLS certificate.
5. **WASM Runtime.** Execute WebAssembly bytecode inside SGX with WASI support (random, sockets, KV).
6. **JWT-Authenticated Vault.** Store and retrieve secrets gated by ES256 JWT verification.
7. **Sealed Config.** All persistent state (CA material, egress CA bundle, KV master key) stored as a single MRENCLAVE-bound blob.
8. **Config Attestation.** A Merkle root and per-module OIDs over all config inputs are embedded as custom X.509 extensions in every RA-TLS certificate.
9. **Trusted Time.** Every time read goes through one choke point that checks the host's answer against a sealed floor, the platform monitor's signed polls and NTS servers, and fails closed for verification decisions when it cannot (see [Trusted Time](/solutions/enclave-os/trusted-time)). WASM apps read it through the WASI clocks.
...
│         │                          │    (4 ECALLs + 1 OCALL)    │
│  ┌──────┴───────────────┐   ┌──────┴─────────────────────────┐  │
│  │  TLS Engine (rustls) │   │  RPC Client                    │  │
│  │  TCP bytes in,       │   │  (encodes req → enc_to_host,   │  │
│  │  HTTP out            │   │   reads responses)             │  │
│  └──────┬───────────────┘   └────┬──────────────┬────────────┘  │
│         │                        │              │               │
│         │                   ┌────┴──────┐  ┌────┴───────┐       │
│         │                   │ HTTPS     │  │ Sealed KV  │       │
...
- `config_leaves()`: named inputs hashed into the [config Merkle root](/solutions/enclave-os/attestation/merkle-tree) OID.
...
|--------|-------|-------------|
| **egress** | `crates/enclave-os-egress` | HTTPS egress client; owns egress root CA store; registers `egress.ca_bundle` config leaf + OID `1.3.6.1.4.1.65230.2.2` |
| **kvstore** | `crates/enclave-os-kvstore` | Sealed (AES-256-GCM) KV store; owns master key from sealed config |
| **wasm** | `crates/enclave-os-wasm` | WASM runtime (wasmtime); registers `wasm.code_hash` config leaf + OID `1.3.6.1.4.1.65230.2.4` (combined workloads hash) |
...
2. **Generates the RA-TLS certificate.** An ECDSA P-256 key pair is generated inside the enclave and wrapped in an X.509 certificate signed by the intermediary CA. The SGX quote that binds the key to the enclave (via `report_data`) is served on demand by the [evidence exchange](/solutions/enclave-os/attestation/ra-tls#the-evidence-exchange), not embedded in the certificate.
...
5. **Makes HTTPS egress requests.** Modules can call out to external services; TLS termination happens inside the enclave, and the host only carries the encrypted bytes over sockets it owns.
...
Root CA (privasys.root-ca.crt)
 └── Intermediary CA (sealed inside enclave)
      └── Leaf RA-TLS certificate (24 h, key kept across re-mints)
               ├── Extension: Runtime Version Hash  (OID 1.3.6.1.4.1.65230.1.1)
               ├── Extension: Config Merkle Root    (OID 1.3.6.1.4.1.65230.2.1)
               ├── Extension: Egress CA Bundle Hash (OID 1.3.6.1.4.1.65230.2.2)  ← egress module
               └── Extension: Workloads Hash        (OID 1.3.6.1.4.1.65230.2.4)  ← wasm module
```

The intermediary CA cert + key are passed to the enclave at init time, sealed to disk as part of the unified `SealedConfig` (MRENCLAVE policy), and kept only in encrypted enclave memory. The leaf certificate is signed by the intermediary CA and renewed every 24 hours. It carries no quote: the SGX quote is served by the evidence exchange (`POST /__privasys/attest`) and bound to the leaf key through `report_data`. Module OIDs are registered by each module via `custom_oids()` and embedded as non-critical X.509 extensions.
...
5. **Module** processes the request. KV operations go over the **RPC Channel**. An outbound HTTPS call goes over a host-owned connection on the Data Channel, and while it waits the request is suspended so the enclave can serve other connections (see [Coroutines](/solutions/enclave-os/enclave-os-mini/coroutines)).
...
See the [Layer 4 Proxy guide](/solutions/enclave-os/enclave-os-mini/guides/layer4-proxy) for Caddy (caddy-l4) and HAProxy configurations, including SNI-based routing for multiple enclave instances.
