...
- **OCI containers** (Enclave OS Virtual): Deploy standard Docker/OCI container images inside a Confidential VM. An attested reverse proxy terminates TLS at the edge, and a workload launcher manages containers via containerd. No changes to your container images are required.

In both cases, clients verify the server's identity through a standard TLS handshake and obtain the attestation evidence on the same connection, so no modified TLS stack or out-of-band verification channel is needed.
...
|-----------|-------------------|
| **RA-TLS everywhere** | Every server presents an enclave-issued X.509 certificate and serves its attestation evidence on the connection. Clients verify the chain in the handshake and the evidence before sending data. |
...
| **Trusted time** | Neither runtime takes its host's clock on trust. The host time is confirmed against a signed floor from the platform monitor, with Network Time Security servers as the tie-breaker, and every verification decision reads that [trusted time](/solutions/enclave-os/trusted-time). |
...
- [Enclave OS Virtual Architecture](/solutions/enclave-os/enclave-os-virtual/architecture): Containers inside Confidential VMs with attested TLS
- [Trusted Time](/solutions/enclave-os/trusted-time): How both runtimes check their host's clock against a platform monitor and NTS
