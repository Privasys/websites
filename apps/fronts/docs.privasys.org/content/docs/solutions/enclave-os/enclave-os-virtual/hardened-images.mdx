...
| Image profile marker | `/etc/privasys/image-profile` (`production` or `dev`), part of the measured rootfs, surfaced in RA-TLS certificates as OID `1.3.6.1.4.1.65230.1.2` |
...
The [data partition](/solutions/enclave-os/enclave-os-virtual/disk-encryption) (LUKS-encrypted, separate persistent disk) survives image updates.
...
The marker lives on the dm-verity-protected rootfs, so it cannot be changed at runtime and a forged value would change the measurement. The [RA-TLS certificate](/solutions/enclave-os/attestation/oids) carries it as the Image Profile extension (OID `1.3.6.1.4.1.65230.1.2`); verification libraries reject `dev` images unless explicitly opted in.
