...
The root of the per-container tree is embedded as OID `1.3.6.1.4.1.65230.5.1` in that container's RA-TLS certificate. A client connecting to `myapp.prod1.example.com` sees only the Merkle root and metadata for `myapp`, not for any other container.
...
The platform Merkle root is embedded as OID `1.3.6.1.4.1.65230.2.1` in the management API certificate.
