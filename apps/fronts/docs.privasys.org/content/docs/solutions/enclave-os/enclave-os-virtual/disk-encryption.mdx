...
3. The key origin (`"vault:<handle>"`, or `"byok:<fingerprint>"` on legacy machines) is written to `/run/luks/dek-origin` and published as **OID `1.3.6.1.4.1.65230.3.1`** in every RA-TLS certificate.

This means a remote verifier can check the RA-TLS certificate's OID `3.1` extension and confirm how the data partition is keyed: vault-held (the default, where the key is gated on the machine's attested identity) or a customer-provided key (legacy BYOK).
...
4. Embeds `"byok:<fingerprint>"` in OID `1.3.6.1.4.1.65230.6.1` of that container's RA-TLS certificate.
...
3. The vaults verify the VM's RA-TLS certificate and the TDX quote it presents through the evidence exchange (checking the RTMR measurements and dm-verity root hash) before releasing key shares.
...
| Level | OID | Value |
|-------|-----|-------|
| Platform (OS data partition) | `1.3.6.1.4.1.65230.3.1` | `"vault:<handle>"` or `"byok:<fingerprint>"` |
| Per-container (volume) | `1.3.6.1.4.1.65230.6.1` | `"vault:<handle>"` or `"byok:<fingerprint>"` |
