...
2. `ReportData = SHA-512( SHA-256(DER public key) || creation_time )`, where `creation_time` is `NotBefore` truncated to minute precision.
...
3. `ReportData = SHA-512( SHA-256(DER public key) || nonce )`.
