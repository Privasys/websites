...
The RA-TLS module is a Caddy `tls.issuance` module that generates RA-TLS certificates inside the Confidential VM. It supports the same [deterministic and challenge-response](/solutions/enclave-os/attestation/ra-tls#deterministic-vs-challenge-mode) attestation paths as Enclave OS Mini.
...
| `1.3.6.1.4.1.65230.3.4` | Container Volume Encryption | `"vault:<handle>"` or `"byok:<fingerprint>"` (present only when an encrypted volume is attached) |
| `1.3.6.1.4.1.65230.3.6` | Container App Id | Platform-assigned app identity (`apps.id`, raw 16-byte UUID), stamped by the measured manager (omitted when the platform supplied none) |
