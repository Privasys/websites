...
A confidential app that needs to keep something for a user already has a durable, confidential place for it: its own encrypted volume, sealed to the enclave and preserved across upgrades and redeploys. It is a disk the app controls: the user reaches what was kept through that app alone, and cannot move it, share it, or take it away as they would a file of their own. Privasys Drive gives an app a folder in the user's own Drive instead. The app asks for a folder by name, the user approves on their wallet, and the app receives a grant to that folder and nothing else. This page describes the consent flow, the grant, and what the user sees.
