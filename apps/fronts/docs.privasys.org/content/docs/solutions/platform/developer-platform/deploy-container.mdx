...
| **Configure endpoint** | Optional `config_api` declaration (`{ method, path }`) describing the in-process endpoint the platform will POST to once at first request, before any other traffic is allowed through. Declared via the `org.privasys.config_api` OCI label or in `privasys.json`. |
| **Encrypted storage** | Toggle to enable an encrypted filesystem volume for persistent data. |

> **Environment variables are no longer accepted.** Per-deployment configuration is delivered in-process via the declared `config_api` endpoint, then frozen by the manager. The container persists what it needs; subsequent restarts replay the same config without going back to the platform.
