...
| `1.3.6.1.4.1.65230.3.6` | Platform-assigned app id (`apps.id`, raw 16-byte UUID) |

This means the certificate attests not just the enclave, but the specific application running inside it. See the [X.509 OID scheme](/solutions/enclave-os/attestation/oids) for the full reference.
...
Beyond the UI, you can verify attestation programmatically using the [verification libraries](/solutions/platform/verification-libraries), available for Python, Go, Rust, TypeScript, and C#:

- **JavaScript / TypeScript** — `@privasys/ra-tls`
- **Go** — `github.com/Privasys/ratls-client-go/ratls`
