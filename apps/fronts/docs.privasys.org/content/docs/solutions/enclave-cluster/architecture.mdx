...
Peer links are mutual TLS 1.3 terminated inside the enclaves, and admission is layered:

1. **Fleet CA + measurement pin.** The peer's certificate must chain to the fleet CA and carry an SGX quote whose measurement is in the admissible set — and the quote must be cryptographically bound to that certificate's own key, so evidence cannot be transplanted between certificates.
...
3. **The credential.** The shared commitment key is generated in-enclave, split across an [Enclave Vaults](/solutions/enclave-vaults/overview) constellation, and released only to enclaves passing the credential policy's checks. No shared secret exists outside TEEs, and *fetching the credential is admission*.
