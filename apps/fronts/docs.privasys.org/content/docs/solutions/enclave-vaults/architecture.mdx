...
Enclave Vaults runs as the `enclave-os-vault` module inside an Intel SGX enclave on top of [Enclave OS Mini](/solutions/enclave-os/enclave-os-mini/architecture). It inherits the OS's attestation stack, sealed storage, RA-TLS ingress, and OIDC token verification.
...
```
               ┌──────────────────────────────────────────────────┐
               │            Vault directory                       │
               │       (management-service, GET /api/v1/vaults)   │
...
| `P256SigningKey` / `Ed25519SigningKey` | Asymmetric key pair, sealed inside the enclave | `Sign` (a message the vault hashes, or a pre-computed digest it signs raw), `Verify` | no |
...
The vault directory models each vault version as a **constellation**: a set of vault instances that share one MRENCLAVE, attestation server, and OIDC configuration. A new vault build is a new constellation, which an operator activates; at most one is active per environment. To roll a new vault enclave version forward we stand up the new constellation alongside the old one and cut the directory pin over to it. When a constellation holds no production keys, the cutover is a clean replacement with fresh sealed storage. When it holds keys the owner needs to keep reachable, the owner stages and promotes the new vault profile on each key first, exactly as for a caller-enclave version change, so the key owner is in control of when their secret material reaches the new vault enclave. Self-migration of sealed storage across a version boundary is on the roadmap.
...
The RA-TLS wire protocol described above is the native surface. On top of it, the same enclave operations are reachable through the interfaces existing software already speaks: a PKCS#11 3.1 module, a KMIP 2.1 gateway, and a REST API plus CLI. These are front-ends over the same policy engine and data plane, so they add no new trust boundary. The [Standard Interfaces](/solutions/enclave-vaults/standard-interfaces) page covers how each maps onto the vault and where the standard client's reach ends.
