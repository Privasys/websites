...
| `enclaves` | `list`, `get` (platform fleet view; requires the `privasys-platform:manager` role) |
| `instances` | `create`, `list`, `describe`, `start`, `stop`, `delete` (dedicated confidential VMs) |
| `volumes` | `list`, `describe`, `resize`, `delete` (encrypted app storage) |
| `account` | `show`, `update` |
