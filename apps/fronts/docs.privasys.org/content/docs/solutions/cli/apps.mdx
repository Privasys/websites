...
The `platform` row is the physical machine's identity read from the verified quote; see [platform pinning](/solutions/enclave-os/attestation/platform-pinning) for pinning it and for revocation.
...
privasys attest my-app --allowed-platform <hex>          # pin the physical machine (the "platform" row; repeatable)
