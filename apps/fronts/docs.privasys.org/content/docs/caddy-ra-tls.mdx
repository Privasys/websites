...
The **Caddy RA-TLS Module** is a [Caddy](https://caddyserver.com/) `tls.issuance` module (`ra_tls`) that produces RA-TLS certificates for **Confidential Computing**. It turns any Caddy instance running inside a Confidential VM into an attested HTTPS server — no custom client-side code required.

**Repository**: [github.com/Privasys/caddy-ra-tls-module](https://github.com/Privasys/caddy-ra-tls-module)
...
The hardware-specific logic is abstracted behind the `Attester` interface ([attester.go](https://github.com/Privasys/caddy-ra-tls-module/blob/main/attester.go)). Each backend lives in its own file (e.g. [attester_tdx.go](https://github.com/Privasys/caddy-ra-tls-module/blob/main/attester_tdx.go)) and registers itself via `init()`.
