---
title: Attested Connections (RA-TLS)
description: How RA-TLS embeds attestation evidence in X.509 certificates, turning every HTTPS connection into a verifiable proof of what code is running.
---
...
Implementations may embed additional custom extensions. For example, [Enclave OS](/solutions/enclave-os/attestation/merkle-tree) adds configuration measurement OIDs that attest the enclave's runtime state alongside the code identity.
...
This is all standard X.509 parsing; any language with an ASN.1 library can extract and verify the quote. Privasys provides [RA-TLS client libraries](/solutions/platform/verification-libraries) in Go, Python, Rust, TypeScript, and C#.
...
For the full specification of configuration measurement OIDs, Merkle tree construction, and verification strategies, see [Enclave OS: RA-TLS Certificates](/solutions/enclave-os/attestation/ra-tls).
...
| Product | Details |
|---------|---------|
| [Enclave OS](/solutions/enclave-os/attestation/ra-tls) | Full RA-TLS with Config Merkle tree, per-app certificates, challenge-response mode |
| [Privasys Platform](/solutions/platform/overview) | RA-TLS certificate issuance for Confidential VMs, deterministic + challenge-response modes |

Both produce standard X.509 certificates verifiable by the same [verification libraries](/solutions/platform/verification-libraries).
