...
Every confidential AI session starts with an RA-TLS handshake. The leaf certificate the client receives is issued inside the enclave (signed by an operator-provisioned intermediary CA whose private key never leaves the TEE), and carries the full TDX evidence plus a small set of Privasys-defined x.509 extensions.
...
| OID | Name | Meaning |
| --- | --- | --- |
| `1.3.6.1.4.1.65230.1.1` | Config Merkle root | SHA-256 root over all platform configuration inputs. |
| `1.3.6.1.4.1.65230.2.x` | Platform extensions | Bind the enclave configuration: combined workloads hash (`2.5`), DEK origin (`2.6`), attestation server set (`2.7`), image profile (`2.8` — `production` or `dev`). |
| `1.3.6.1.4.1.65230.3.5.5` | `MODEL_DIGEST` | The dm-verity root hash of the model disk currently mounted, or - if no verity disk is in use - a deterministic SHA-256 over the model's index file. Lives under the app arc `3.5.*`; also emitted at the legacy literal `3.5` while older clients still pin it there. |
| `1.3.6.1.4.1.65230.3.5.7` | `TOOLS_DIGEST` | SHA-256 over the canonical, sorted JSON of the configured MCP tool servers, so a verifier can prove exactly which tools the AI enclave exposes. Lives under the app arc `3.5.*` because it is an application extension, not a platform-standard OID (older fleet images emitted it at the top-level `3.7`). |
| `1.3.6.1.4.1.65230.3.x` | Workload extensions | Per-workload bindings: config root (`3.1`), image digest (`3.2`), image reference (`3.3`), key source (`3.4`), platform-assigned app id (`3.6`). |
