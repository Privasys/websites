...
The whole ceremony in one picture:

```mermaid
sequenceDiagram
  participant P as App page + SDK iframe
  ...
  W->>E: RA-TLS handshake, verify attestation
  W->>E: POST /__privasys/session-bootstrap { sdk_pub }
  E-->>W: { session_id, enc_pub, expires_at }
  W->>W: FIDO2 assertion over the binding challenge<br/>(nonce, sdk_pub, quote_hash, enc_pub, session_id)
  W->>I: assertion (via the broker, opaque frames)
  I->>I: recompute challenge, constant-time compare
  I-->>P: JWT: att_verified, att_quote_hash, att_oids, session.*
  P->>P: pin policy, derive K = HKDF(ECDH(sdk_priv, enc_pub))
  P->>E: sealed frames, AES-256-GCM under K
  E-->>P: sealed responses
  Note over W,E: the wallet brokered the exchange yet cannot derive K:<br/>it never held sdk_priv
```
