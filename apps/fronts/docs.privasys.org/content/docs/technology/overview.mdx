...
The fastest way to try Privasys is the [CLI](/solutions/cli). Four commands take you from nothing to a deployed, hardware-attested confidential app:
...
privasys apps deploy my-app --watch    # deploy to confidential infrastructure
privasys attest my-app                 # verify the hardware quote yourself
```

The CLI is also an [MCP server](/solutions/cli/agents), so an AI agent can drive the whole platform: run `privasys agents init` in your project and ask your agent to deploy a confidential app.
...
Confidential AI you can verify: open-weight models running inside Intel TDX confidential VMs with NVIDIA H100 in confidential-compute mode, with private-data memory through Privasys Drive and agents over Attested MCP, all inside the trust boundary. The runtime and attestation surface are documented under [Confidential AI](/technology/confidential-ai/overview) in this section.

[Privasys AI documentation](/solutions/ai/overview)

### Privasys Platform

The deployment infrastructure that ties everything together: a [developer platform](https://developer.privasys.org) for deploying WASM modules and containers on confidential infrastructure, the [`privasys` CLI](/solutions/cli) (which is also an MCP server for AI agents), verification libraries in multiple languages (Python, Go, Rust, TypeScript, C# .NET), and an attestation server that independently verifies hardware quotes. The Developer Platform is live, sign in with GitHub and deploy your first application in minutes.

[Platform documentation](/solutions/platform/overview)

### Privasys ID

The identity and authentication layer of the platform. Privasys ID groups the mobile [Wallet](/solutions/privasys-id/wallet) (FIDO2 authenticator with built-in RA-TLS verification, hardware-bound keys in Secure Enclave / StrongBox), the [IdP](/solutions/privasys-id/idp) at `privasys.id` (OIDC issuer with attestation-bound claims), the browser [Auth SDK](/solutions/privasys-id/auth-sdk) (`@privasys/auth`, iframe SSO across the ecosystem), and the [Sealed Session](/technology/sealed-session) mechanism that brings attestation-verified end-to-end-confidential transport to clients that cannot speak RA-TLS themselves — starting with browsers.
...
<Cards>
  <Card title="Privasys CLI" href="/solutions/cli">
    Deploy, attest, and manage confidential apps from your terminal, CI, or an AI agent.
  </Card>
  ...
  <Card title="Privasys AI" href="/solutions/ai/overview">
    Verifiable confidential inference, private-data memory, and agents inside the trust boundary.
  </Card>
  ...
  <Card title="Privasys ID" href="/solutions/privasys-id/overview">
    Wallet, IdP, browser SDK, and sealed-session transport — the identity and authentication layer.
  </Card>
</Cards>
