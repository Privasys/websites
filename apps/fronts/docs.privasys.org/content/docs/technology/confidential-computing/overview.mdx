...
Intel SGX is available on select Intel Xeon processors (Ice Lake, Sapphire Rapids, and later) and is the foundation for Privasys's [Enclave OS (Mini)](/solutions/enclave-os/architecture).
...
- **configfs-tsm.** Quote generation via the kernel's `/sys/kernel/config/tsm/report` interface, requiring no special SDK.
...
The [Privasys Platform](/solutions/platform/overview) currently supports Intel TDX, with AMD SEV-SNP support planned.
