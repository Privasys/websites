...
```mermaid
sequenceDiagram
  participant N as New version (enclave)
  participant V as Vault constellation
  participant O as Owner

  N->>V: reconstruct DEK, live quote over RA-TLS
  V->>V: measurement vs owner's policy: mismatch
  V-->>N: denied, volume stays sealed (the gate)
  O->>V: promote the staged measurement<br/>(owner token + step-up approval)
  N->>V: retry, same quote
  V-->>N: shares released, DEK reconstructed in-enclave
  N->>N: volume unlocks, data readable
  Note over O,V: the owner, not the platform,<br/>decides which code reads the data
```
