...
sequenceDiagram
  ...
  participant S as Harbour Bets (Auth SDK)
  participant I as IdP (privasys.id)
  participant W as Wallet
  participant V as Identity-verifier enclave

  C->>S: clicks "Verify my age"
  S->>I: opens the Privasys ID gate (identity scope)
  I->>I: reserve Harbour Bets' credits, mint a disclosure voucher<br/>(names the client and the claim, never the user)
  I->>W: QR / push rendezvous
  W->>C: per-attribute consent screen<br/>(first visit: free passport verification)
  C->>W: biometric approval
  W->>V: proof + voucher, over RA-TLS
  V->>V: re-open the birthdate commitment, check the holder's<br/>hardware-key signature, verify the voucher
  V-->>S: signed SD-JWT VC (age_over_18), the voucher settles
  S->>S: verify server-side, store the receipt, unlock
```
...
Relying-party registration is operator-assisted today: [contact us](mailto:contact@privasys.org) and we register the client with you during onboarding (self-serve registration from the developer portal is on the roadmap). Registration fixes three things:
...
3. The billing link: your client is flagged billable and tied to your platform billing account. You buy credits on [developer.privasys.org](https://developer.privasys.org) as usual. Gov-verified disclosures are paid per answer; a client that is not billable (or is out of credits) is rejected at the toll booth **before the user is ever prompted**.

<Callout type="warn">
Request the *derived* claim, not the raw data. `age_over_18` gives you a boolean. Requesting `birthdate_id` would also work technically (it is a certifiable attribute), but then you are back to holding a date of birth you must protect. The entire point of the scheme is that the minimal claim is the strongest position: less PII, same legal force. Note the spelling: `birthdate` on its own is the date the holder typed into their wallet, and no passport ever confirmed it.
</Callout>
...
Because each disclosure is individually priced, your compliance cost is exactly proportional to verified sessions; see [pricing](https://developer.privasys.org/pricing) for the current per-attribute rates. Three properties of the platform's fee system are worth knowing as the payer:
