...
So the question is *how* attestation should reach those clients, not *whether*, given that they are structurally incapable of doing the verification themselves.
...
You are extending the *reach* of the existing trust model, not the model itself. The wallet was already the verifier for sign-in. It is now the verifier for the session.
