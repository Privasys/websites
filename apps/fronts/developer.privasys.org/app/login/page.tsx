'use client';

import Link from 'next/link';
import Image from 'next/image';
import { signIn, useSession } from 'next-auth/react';
import { useRouter } from 'next/navigation';
import { useEffect, useState } from 'react';

export default function LoginPage() {
    const { status } = useSession();
    const router = useRouter();
    const [loading, setLoading] = useState<string | null>(null);

    useEffect(() => {
        if (status === 'authenticated') router.replace('/dashboard');
    }, [status, router]);

    if (status === 'loading' || status === 'authenticated') {
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="animate-pulse text-sm text-black/50 dark:text-white/50">Redirecting…</div>
            </div>
        );
    }

    return (
        <div className="min-h-screen flex flex-col items-center justify-center px-6">
            <Link href="/" className="flex items-center gap-2.5 mb-10">
                <Image src="/favicon/favicon.svg" alt="" width={24} height={24} aria-hidden />
                <span className="text-xl font-semibold">Privasys Developer</span>
            </Link>

            <div className="w-full max-w-sm p-8 rounded-2xl border border-black/10 dark:border-white/10">
                <h1 className="text-xl font-semibold text-center">Sign in</h1>
                <p className="mt-2 text-sm text-center text-black/50 dark:text-white/50">
                    Sign in to access the developer platform.
                </p>

                <button
                    type="button"
                    disabled={loading !== null}
                    onClick={() => {
                        setLoading('privasys');
                        signIn('privasys', { callbackUrl: '/dashboard' });
                    }}
                    className="mt-8 w-full flex items-center justify-center gap-2.5 px-4 py-2.5 text-sm font-medium rounded-lg bg-black text-white hover:bg-black/80 dark:bg-white dark:text-black dark:hover:bg-white/80 transition-colors disabled:opacity-50"
                >
                    <Image src="/favicon/favicon.svg" alt="" width={18} height={18} aria-hidden />
                    {loading === 'privasys' ? 'Redirecting…' : 'Sign in with Privasys Wallet'}
                </button>

                <div className="relative my-6">
                    <div className="absolute inset-0 flex items-center">
                        <div className="w-full border-t border-black/10 dark:border-white/10" />
                    </div>
                    <div className="relative flex justify-center text-xs">
                        <span className="bg-white dark:bg-black px-2 text-black/40 dark:text-white/40">or</span>
                    </div>
                </div>

                <button
                    type="button"
                    disabled={loading !== null}
                    onClick={() => {
                        setLoading('zitadel');
                        signIn('zitadel', { callbackUrl: '/dashboard' });
                    }}
                    className="w-full flex items-center justify-center gap-2.5 px-4 py-2.5 text-sm font-medium rounded-lg border border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5 transition-colors disabled:opacity-50"
                >
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                        <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.694.825.576C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
                    </svg>
                    {loading === 'zitadel' ? 'Redirecting…' : 'Continue with GitHub'}
                </button>

                <p className="mt-6 text-xs text-center text-black/40 dark:text-white/40">
                    By signing in, you agree to the{' '}
                    <a href="https://privasys.org/legal/terms" target="_blank" rel="noopener noreferrer" className="underline">Terms of Service</a>
                    {' '}and{' '}
                    <a href="https://privasys.org/legal/privacy" target="_blank" rel="noopener noreferrer" className="underline">Privacy Policy</a>.
                </p>
            </div>
        </div>
    );
}
